...
impl MediaPlayer for DebugPlayer {
    fn name(&self) -> &str {
        match self {
            ...
            DebugPlayer::Socket(_s) => {
                "Socket-Based Text Debug"
        ...
        for stream in s.connection_info.iter_mut() {
            ...
            for ln in msg.lines() {
                ...
                if ln.starts_with('p') {
                    ...
                    let time_portion = ln.splitn(2, ' ').nth(1)
    ...
    fn ping(&self) -> MyResult<TimePing> {
        match self {
            ...
            DebugPlayer::Socket(s) => {
                let dt = if s.is_paused {Duration::from_nanos(0)} else {Instant::now() - s.last_position_modification};
...
impl MediaPlayerList for DebugPlayerList {
    ...
    fn available(&mut self) -> MyResult<Vec<String>> {
        Ok(vec!["Standard Output".to_owned(), "Log File".to_owned(), "Socket based".to_owned()])
    }
    fn open(&mut self, name: &str) -> MyResult<Self::Player> {
        match name {
            "Standard Output" => Ok(DebugPlayer::Sink(SinkDebugPlayer::new(DebugEventSink::StdOut))),
            "Log File" => {
                ...
                let file = fs::OpenOptions::new()
                    ...
                    .create(true)
                    .open(path)?;
                Ok(DebugPlayer::Sink(SinkDebugPlayer::new(DebugEventSink::File(file))))
            }
            "Socket based" => {
